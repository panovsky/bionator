...
function preload() {
  ...
  if(lang == 'de'){
    $.get('assets/data.json', function(data) {
      for (var i in data.langs[0]) { // 0 - for doctor eng, 1 - for doctor deutsch, 2 - patient eng, 3 - patient deutsch
            txts[i]=(data.langs[0][i]); // 0 - for doctor eng, 1 - for doctor deutsch, 2 - patient eng, 3 - patient deutsch
